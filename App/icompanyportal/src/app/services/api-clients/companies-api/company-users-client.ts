...
import { NotifyRequest } from '../notifications-api/notify-request';
...
export class CompanyUsersClient {
  ...
  getAll(companyId: number) {
    return this.http.get<CompanyUser[]>(
      `${this.endpoint}/users/${companyId}/all`
    );
  }
  ...
  notify(companyId: number, request: NotifyRequest) {
    return this.http.post<void>(
      `${this.endpoint}/users/${companyId}/notify`,
      request
    );
  }
  ...
  delete(companyId: number, userId: number) {
    return this.http.delete<void>(
      `${this.endpoint}/users/${companyId}/${userId}`
    );
  }
}
