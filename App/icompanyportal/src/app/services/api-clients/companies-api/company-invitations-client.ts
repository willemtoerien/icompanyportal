...
export class CompanyInvitationsClient {
  ...
  getInvitations(companyId: number) {
    return this.http.get<CompanyInvitation[]>(
      `${this.endpoint}/invitations/${companyId}/all`
    );
  }

  get(token: string) {
    return this.http.get<CompanyInvitation>(
      `${this.endpoint}/invitations/${token}`
    );
  }
  ...
  invite(
    ...
    return this.http.post<void>(
      `${this.endpoint}/invitations/${companyId}/${encodeURIComponent(
        responseUrl
      )}`,
      request
    );
  ...
  respond(token: string, accepted: boolean) {
    return this.http.put<void>(
      `${this.endpoint}/invitations/${token}/${accepted}`,
      undefined
    );
  }
